...
cp "benchmark/"$testSystem"/mutants/"$mutatedFile $replacePath
cd ..
...
do
  ...
  if [ ! -d "results/"$testSystem"/"$map"/benchmark"$benchmarkNum"/"$segmentsFolder"/reg" ]
  then
    mkdir "results/"$testSystem"/"$map"/benchmark"$benchmarkNum"/"$segmentsFolder"/reg"

  fi

  saveFolder="results/"$testSystem"/"$map"/benchmark"$benchmarkNum"/"$segmentsFolder"/reg"
...
cp "benchmark/"$testSystem"/original/"$mutatedFile $replacePath
cd ..
./scripts/bootstrap.sh stop
./apollo.sh build_opt_gpu
./scripts/bootstrap.sh
